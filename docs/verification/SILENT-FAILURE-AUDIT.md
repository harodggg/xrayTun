...
**A-1 与 A-2 的修复点在 `crates/xt-tun`（macOS 侧的 `rollback` / `force_cleanup`）与
`crates/xt-helper`（`server.rs` 的响应构造）—— 两处都是 **helper 侧**。**
⇒ 与 v0.8.34（只改 App、**不需要**重装助手）不同：**下一版（v0.8.35）用户必须重装特权助手**，
否则他更新完 App 之后，机器上跑的还是**旧 helper**，A-1/A-2 一点都不会生效
（App 侧那条「回滚失败」分支依然永远不会被触发）。
**判断依据**：`git diff --stat v0.8.34..<fix> -- crates/xt-helper crates/xt-tun` **非空** ⇒ helper 侧变了（我核对过范围）。
