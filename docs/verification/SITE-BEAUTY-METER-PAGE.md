...
# v1.2.0：角标左上角的 AI 标识（悬停直接检测）+ 两个真 bug

## 用户反馈与改动

1. 反馈「没有显示是否 AI」→ 查出两个真问题：
   * **AI 区块在卡片/弹窗最底部**：卡片 `max-height: min(76vh, 620px)` + `overflow: auto`，
     不滚动看不到；已移到分数摘要之后、五个维度之前，并加了两条端到端回归检查
     （卡片不滚动即可见、弹窗 600px 首屏内）。
   * **设置页两个 AI 开关完全无效**：后台 `normalizeSettings` 没登记 `aiDetection` / `aiBadgeAuto`
     ⇒ `GET_SETTINGS` 与 `SETTINGS_CHANGED` 都不带这两个键，界面永远拿默认值。
     已补登记，并加了一条单元测试盯住「UI 消费的设置键必须在后台登记齐全」这类问题。
2. 要求「增加到标识的左上角」+「直接调用 AI 检测」→ 角标**左上角**新增 AI 标识，且默认
   **悬停即自动检测**：首次约 1~3 秒加载 52.5MB 模型（标识显示 `AI …`），之后每张新图约 0.8 秒；
   同一 URL 走 LRU 缓存不重复推理；设置页可关掉自动，回到「可点的 `AI?`，点了才推理」。

## 验证（v1.2.0）

| 检查 | 结果 |
| --- | --- |
| 单元测试 + 完整性 | **38 / 38**（含设置键登记契约、AI 纯逻辑、清单/模型完整性） |
| 真实 Chrome 端到端 | **30 / 30**：角标左上角标识位置、**悬停自动出结果（未点击）**、缓存复用不再推理、关闭自动后停在 `AI?` 且点击出结果、卡片不滚动可见、弹窗首屏可见、offscreen 生命周期 |
| 打包产物本身 | 解压到临时目录再跑一遍真机端到端：**29 / 29**（当次运行快照，含标识与截图检查） |
| `gen-site-jsonld.py check` / `gen-site-geo.py check` | 全绿（8 页；四份产物逐字节一致） |
| 本地 HTTP 与静态检查 | 两个页面 + 全部相对引用 200；`site/` 无 >25 MiB 文件；完整版 zip 在 Releases（50,031,764 字节） |

新增截图：`site/beauty-meter/preview-badge.png`（角标左上角绿色 `AI 0%` 标识的真实截图）。
