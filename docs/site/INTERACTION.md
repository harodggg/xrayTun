...
|---|---|---|---|
| 1 | 点「下载 dmg」 | 按钮旁边写**版本号 + 大小 + 文件名**：`v0.8.28 · 47.1 MB（45.0 MiB）· XrayTun_0.8.28_x86_64_arm64.dmg`，并给 zip 备用链接与 SHA256 入口 | **【历史·当时】不是今天该填的值，别抄**：【实测·真机】`gh release view v0.8.26`：dmg = 47,128,987 B、zip = 42,636,964 B、`SHA256SUMS.txt` 200 B、`isDraft: false`、`publishedAt: 2026-09-20` |
...
* **INT-2-2**：下载 URL 用**具体 tag 的资产 URL**（`…/releases/download/v0.8.28/XrayTun_0.8.28_x86_64_arm64.dmg`），
...
* **INT-4-1（采用 C）**：
  ...
  * JS 拉 `https://api.github.com/repos/harodggg/xrayTun/releases/latest`，**只做一件事**：
    ...
    「仓库已有 vX.Y.Z（本页显示的 v0.8.28 仍是可下载的）」；
...
| **D 正常** | 主按钮：`下载 macOS 版（v0.8.28 · 45.0 MiB）`；次链接：zip、SHA256、Release 页 | INT-2-1/2-2 |
...
* **INT-5-5（屏幕阅读器）**：
  * 一个 `<h1>`，层级 `<h2>`/`<h3>` 不跳级；
  * 每个 `<a>` 有可读文本（禁止「这里」）；
  * 下载按钮的**可访问名**要含版本：`下载 macOS 版 XrayTun v0.8.28（dmg，45.0 MiB）`；
...
5. **文件大小换算**：47,145,126 B = 45.0 MiB = 47.1 MB；42,647,148 B = 40.7 MiB = 42.6 MB（按 1 MiB=1048576 计算）。
   官网呈现方式由 `art-designer`/`product-manager` 定，但**必须与 `gh release view` 的字节数一致**。
6. **版本号会过期**：本文中**陈述当前事实**的字段取自 `gh release view v0.8.28`
   （2026-09-20T16:03:44Z，`isDraft: false`）；标注 **【实测·真机】/【实测·浏览器】** 的段落是
   **当时对 v0.8.26 产物的观测记录，保留原样、不代表当前版本**。
   发新版后需同步更新 §2.1、§4 中的数字。
   （`site/**` 那边已由 `scripts/check.sh` 的「站点版本一致性」检查兜住；**本文档还是手写**，
   改版时仍需人工同步 —— 本行就是被 task-34 发现漂移后更新的。）
