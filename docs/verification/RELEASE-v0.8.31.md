...
**结论**：这是「**正在发布**态页面上的一处陈旧数字**」，属 `site/**`，我**只报告不改**（已发给 `ops`；`ops` 已改为「大小见 Releases」并重跑生成器 —— **我重跑复核 = 0 条，见 §1.7**）。
...
**后续（§1.7）**：`ops` 已补写 `## 0.8.31` 条目，我重跑 `grep -c` = **1** ✅。
> 关于计数口径的一个说明：`ops` 问「正文是否也要复读版本号」。我的口径是**不要求** ——
> 我量的是「**条目存在**」，用 `grep -c "0\.8\.31"` ≥1 只是最省事的代理；历史上的 0.8.30 条目同样不在正文复读版本号。
> 真正要防的是「点进去还是上一版」，而这由**顶部条目名**决定。

### 1.7 两处修复与我**重跑**的复验（不采信「已修」）

`ops` 回复 §1.3 / §1.6 已改。我**重跑了受影响的全部检查**（并因为页面变了，把 §2 的逐字节验证**也重跑**）：

**原始输出**：

```
grep -rn "45\.0 MiB\|40\.7 MiB" site/                → 0 条                    ✅（§1.3 已修）
grep -ro "0\.8\.30" site/ | wc -l                    → 0                       ✅
grep -rn "releases/download" site/ | wc -l           → 0（仍无 pinned 直链）   ✅
grep -c "0\.8\.31" CHANGELOG.md                      → 1（顶部已有 `## 0.8.31`）✅（§1.6 已修）

CTA 新文案（我直接读的原文）：
site/index.html:250  >下载 macOS 版 v0.8.31 · dmg（大小见 Releases）</a>
site/index.html:254  >备用 .zip（大小见 Releases）</a>
site/en/index.html:251 >Download for macOS v0.8.31 · dmg (size per Releases page)</a>

6 条版本断言重新复现：gen-site-jsonld.py=0.8.31 / gen-site-geo.py=0.8.31 / site.js=0.8.31 /
gen-site-images.py=0.8.31 / index.html dmg=0.8.31 / en/index.html dmg=0.8.31；PUBLISHED=False ×2 ✅
```

**重跑的决定性验证（因为页面改了，旧结论不能沿用）**：

```
〔JSON-LD〕副本 `gen` 后与仓库逐字节比对：
  IDENTICAL site/index.html · site/en/index.html · site/wasm/index.html · site/en/wasm/index.html

〔GEO 产物，本次扩充〕副本跑 `gen-site-geo.py` 后与仓库逐字节比对：
  IDENTICAL site/llms.txt        IDENTICAL site/llms-full.txt   ← §1.3 那 4 处 MiB 已随之消失
  IDENTICAL site/robots.txt      IDENTICAL site/sitemap.xml
```

⇒ **两处修复独立复验通过**；并且**两个生成器的产物都恰好等于生成器自己的输出**（既没漏跑 `gen`，也没有手改残留）。

> ⚠️ **口径提醒**：以上仍是「**提交 1 尚未提交**时的共享工作区」读数（`git log -1` 仍为 `035e083`）。
> 提交 1 落成 commit 后，我会把 §1 整节**重跑一遍并改绑修订号**；在此之前这些**不是**验收数字。
...
- [x] **复核 §1.6**：`ops` 已补 `## 0.8.31` 条目，我重跑 `grep -c "0\.8\.31" CHANGELOG.md` = **1** ✅（§1.7）
- [ ] 复核 §1.3 的字节数：**提交 2** 时那 6 处 CTA 应填回 **0.8.31 的真实字节数**（此刻写「大小见 Releases」是本阶段的正确答案；若提交 2 后仍只有占位文案 = 漏填）
...
| # | 执行者自述 | 我的实测 | 判定 |
|---|---|---|---|
| 1 | 「陈旧字节数已清空/不再展示」 | 我抓到的那一刻：`*_BYTES/*_MIB` 常量**确实**清空，但**页面 10 处仍在展示 0.8.30 的真实 MiB**（§1.3）；`ops` 随后已修，我重跑 `grep` = **0 条**（§1.7） | ⚠️ **曾不一致 → 已复验通过** |
