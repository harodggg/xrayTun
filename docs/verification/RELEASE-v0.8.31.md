...
```
  ...
  ⇒ **结论**：122 与 68 是**两个不同对象**（上一版的页面 vs 这一版的页面）上的净计数，
  差额 54 **不需要**等于「撤掉的部分」——因为提交 1 同时也在新文案/JSON-LD/描述里**新增**了 `v0.8.31`。
  `ops` 的**方向**（撤 pinned URL 是主因）站得住，但**那个「54 = 撤掉的数量」的算术不成立**，我按实测记 65 / 4 / 净差 54。
  （这正是我在 §0 里盯 `ops` 那处 18 文件/+681 的同一条纪律。）
...
**已提前测的对照基线（此刻线上还是旧版本，因为提交 1 未 push/未部署）**：

```
curl -s https://xraytun.top/ → 43987 bytes
  0.8.31 出现 0 次    0.8.30 出现 35 次    「正在发布」出现 0 次
...
| 3 | 「122 → 68，差 54，因为撤掉 pinned URL 与 og-image 文件名」 | `0.8.30`@`035e083`；其中 pinned 行 **65** 次、og 行 **4** 次；**65+4=69 ≠ 54**。净差是两个不同页面上的计数差，不宜这样分解 | ⚠️ **方向对、算术不成立**（我按 65/4/净差 54 记） |
