...
把「流量被代理覆盖到什么程度」编码成 5 个 tone。前四个是上面语义色的**别名**
（不新造色，只给语义一个名字），只有 `--status-off` 是新值：

| token | 值 | 用途 | 线 vs `--bg` |
|---|---|---|---|
| `--status-on` | `var(--ok)` | 整机受保护（TUN 在跑且路由已接管） | 9.57:1 |
| `--status-partial` | `var(--accent)` | 部分覆盖（系统代理在跑） | 5.73:1 |
| `--status-off` | **`#566a88`** | **没有代理覆盖**（直连 / 未运行）—— 也是 `.topbar` 的兜底值 | 3.34:1 |
| `--status-busy` | `var(--warn)` | 正在自愈 / 隧道建好但路由未接管 | 11.02:1 |
| `--status-failed` | `var(--danger)` | 代理承诺已破（退回直连 / 核心报错） | 6.65:1 |

> 完整语义表、判据字段、优先级与无障碍做法见 **§2.7**。
...
### 2.7 状态线语义（task-45，**已实现**）

顶栏底边那条 **2px** 的线横跨整窗、常驻可见，是应用里最「环境化」的信号。
它必须回答**一个问题**：*此刻这台机器的流量被代理覆盖到什么程度？*

**它以前是一句假陈述**：`styles.css` 把 `.topbar` 的底边**写死**成 `2px solid var(--ok)`，
全项目 `grep 'topbar--'` **0 命中** —— 那条线从来没跟状态变过。于是
**直连、系统代理、未连接、恢复中、已退回直连全都是绿的**。
而在这套配色里绿色 = 已受保护，所以直连模式下等于谎报「你受保护了」（实际毫无代理）；
自动兜底退回直连时更严重：**流量已经在裸奔，界面还在报平安**。

#### 五种 tone

| tone | 含义 | 判据（**全部是后端真实字段**） | 颜色 | 线 vs `--bg` 对比度 |
|---|---|---|---|---|
| `on` | **整机受保护** | `mode === "tun"` && `running` && `routes_committed` | `--status-on` = `--ok` 绿 | **9.57:1** |
| `partial` | 部分覆盖 | `mode === "system_proxy"` && `running` | `--status-partial` = `--accent` 蓝 | **5.73:1** |
| `off` | **没有代理覆盖** | `mode === "direct"`；或 `!running`（非报错） | `--status-off` `#566a88` 中性灰 | **3.34:1** |
| `busy` | 正在自愈 / 路由未接管 | `recovery.phase === "recovering"`；或 `mode === "tun"` && `!routes_committed` | `--status-busy` = `--warn` 琥珀 | **11.02:1** |
| `failed` | **代理承诺已破** | `recovery.phase === "failed"`；或 `lastError && !running` | `--status-failed` = `--danger` 红 | **6.65:1** |

#### 六条硬性约束

1. **绿色只属于 `on`。** 直连、系统代理、未连接、恢复中、退回直连**一律不得是绿色** ——
   这是本条的原始缺陷，测试里有一条专门的**反例**断言 CSS 里只有 `.topbar--on` 映射到 `--status-on`。
2. **默认值是 `off`（中性灰），不是绿。** 漏配状态、快照还没到、将来新增模式分支时，
   默认必须是「不可信」而不是「已受保护」。`.topbar` 的兜底色因此是 `--status-off`。
3. **判据只来自真实字段**：`settings.mode` / `runtime.running` / `runtime.routes_committed` /
   `runtime.last_error` / `runtime.recovery`（经既有的纯函数 `recoveryView` 翻译）。
   **不允许前端按时间、次数或 notice 文案猜。**
4. **优先级：故障压过模式。** `direct_fallback` 时 `mode` **仍然是 `"tun"`** ——
   若按模式取色就会画成绿色，而那正是这条缺陷最严重的一种表现。
5. **`routes_committed` 只对 TUN 生效。** 它是 TUN 两阶段启动的闸门
   （`supervisor.rs` 里提交路由那段在 `if mode == Tun` 分支内）；系统代理不接管路由，
   拿它压系统代理会把「部分覆盖正常工作中」误报成「流量还没走代理」。
6. **状态线与它右边那个小圆点同色。** 两者相距几像素、说的是同一件事，
   两处不一致比一处错更难查。`App.tsx` 用同一个 `tone` 同时驱动 `.topbar--*` 与 `.dot--*`。

#### 为什么 `--status-off` 是新值，不是复用 `--border`

`--border #263148` 对 `--bg` 只有 **1.42:1**、`--border-strong #334155` 只有 **1.78:1** ——
**都低于 WCAG 1.4.11 对「承载信息的图形」要求的 3:1**。这条线承载状态信息，不是装饰。
`#566a88` 实测 **3.34:1**（对 `--bg`）/ **3.06:1**（对 `--bg-elevated`），
且刻意选成**最暗**的一档：明度顺序本身也带信息（**暗 = 没有覆盖**）。

#### 无障碍：颜色不是唯一载体

那条线**没有任何文字**，色盲用户或灰度截图里读不出五种色调。落地做法是**两条一起**：

- `title` 挂在 `<header>` 上（鼠标悬停可读）；
- 一个 `.sr-only` + `role="status"` 的文本节点，内容与颜色同源（`status.detail`），
  状态变化时由屏幕阅读器播报。**新增了 `.sr-only` 工具类**（此前全站没有）。

> **已知残留**（不在 task-45 写入范围内，报给 lead）：
> `pages/Dashboard.tsx:68-88` 的状态词派生**不看 `mode`** ——
> 系统代理与 TUN 都显示成绿色的「已连接」。所以顶栏在系统代理时是**蓝**、
> 状态区文字是**绿**，两处对同一个事实给了不同强度的说法。
> 让状态区采纳同一个 tone 需要改 `Dashboard.tsx`（超出本任务写入范围）。
> 另：`.badge--ok` 渲染出来是 `--warn`（琥珀）色 —— 名实不符，见 §1.3(a)。
