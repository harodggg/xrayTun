...
## 11. 新增需求：**自动检测「最新版本」**（用户 2026-09-24）

### 11.1 需求与口径（Lead 定）

用户原话：**「增加自动检测最新的版本」**。现状（Lead 取证）：`xt_core::update::check_app(proxy)` 与
`check_app_update` 命令都在，但**只有手动按钮会调**（`Settings.tsx:1230` / `:1316`）；启动流程里
`bootstrap` / `reconnect_if_needed` / `run_dns_probe_bg` 都是后台非阻塞，**唯独没有任何版本检查**
⇒ 用户不打开设置就永远不知道有新版本。

定下的三条口径：
1. **只检测，不自动下载/安装** —— 安装仍是用户点击（`install_app_update` 一字未动）；
2. **失败必须说「没查到」**：任何文案与日志都**不许**出现「已是最新」，失败也**不清掉**上次成功查到的版本；
3. **没连隧道也要能查**（传 `None` 直连），否则「系统代理 / 直连」模式下永远检测不到，等于没做。
4. **三路都要自动化**（`task-196` 补的）：客户端 / **核心** / **geo** —— 只做客户端等于
   「核心有新版」永远要用户自己去点，那就不叫「自动检测最新的版本」。

### 11.2 落地的东西（全部在 `origin/main`）

| 卡 | commit | 内容 |
|---|---|---|
| `task-188` | `674f25d` | **本体**：启动 **20 s** 后台查一次 + **每 6 h** 复查（`interval` 跳过第一跳 + 每轮再过一道 `should_check`）；代理「在跑走节点 / 没在跑直连」；失败只写错误、不清 `latest_app`、不刷屏（状态变化才记一条）；新模块 `apps/desktop/src/version_check.rs` |
| `task-189` | `924812f` | 仪表盘状态区一条提示（**不打开设置页也看得见**），可点跳「设置 → 核心与数据更新」 |
| `task-190` | `fe8c608` | 组合态：**曾查到有新版 + 最近一次复查失败** ⇒ 仍显示「有新版本 vX」并如实带限定语（不再把已知事实藏起来） |
| `task-192` | `e1de2ec` | `app://update-checked` 的**真实消费者**（复用既有 `refresh()`）⇒ 自动查到的结果不再要等下次快照刷新。**刻意没有**用「发个假 `nodes://changed` 骗刷新」那种语义撒谎的做法 |
| `task-191` | `d12bee9` | 拆出**客户端专属**的 `check_error_app` / `checked_at_app`；并补两条跨语言契约（见 §11.3） |
| `task-193` | `7cebbbd` | chip 改用客户端专属字段（核心/geo 的失败不再把「客户端已是最新」说成「不知道有没有新版本」——under-claim） |
| `task-194` | `259e329` | **功能性修复**：设置页三处「客户端结论」改用 `check_error_app` —— 其中 `Settings.tsx:1330` 的安装按钮门原来是合并 `check_error`，**核心/geo 一失败「更新到 X 并重启」按钮就消失**（改前/改后真实 DOM：`HAS_INSTALL_BTN=false → true`）。banner 同时开始标明失败来自哪一类 |
| `task-195` | `2f7d484` + `03bd1ff` | 补齐 **core/geo 专属**错误字段（geo 的失败此前**没有任何字段可写** ⇒ 界面永远看不到）；`check_error` 改为**派生**（客户端 → 核心 → geo 取第一条非空）⇒「任一路成功都不清除别人的失败」由构造保证；`03bd1ff` 修掉由此引出的 banner 误标（geo 的失败曾被说成「核心」） |
| `task-196` | `d98d9b6` | **自动检测原来只覆盖客户端** ⇒「核心有新版」「geo 有新版」仍然只有手动按钮会查。现在每轮一次查**三路**（`check_now` + `check_core_geo_now`），落盘走生产/测试共用的 `apply_round`；三路**各自独立**（谁失败在谁那一格里说话，不 fail-fast）；core/geo 的日志与客户端同口径（第一次失败 / 原因变了 / 恢复各一条）。附**生产源级守卫**：`check_once` 少任何一路都会红 |

**这一波顺手抓到的功能性回归**（`task-194` 由 UI 同学的 `rg 'check_error'` 清点发现，不是计划内的）：
安装按钮的显示条件是 `!check_error`（三类共用）⇒ **核心/geo 的检查一失败，即使用户确证有新版，按钮也消失** ——
「已知有新版却无从安装」。改后该按钮在该场景**出现且可用**（真实渲染 DOM 证据在 commit 里）。

### 11.3 这一波最重要的**机制**产出：三条跨语言契约

前两条是「本来以为有、其实没有」的守卫：

1. **`UpdateStatus` / `AvailableUpdate` 的嵌套字段**：`type_contract.rs` 原先只比 `AppSnapshot` 的**顶层**键名
   ⇒ 后端给 `UpdateStatus` 加字段而忘改 `types.ts` **不会有任何测试变红**，界面读到 `undefined`，
   而 `undefined` 是 falsy ⇒ **静默退回老路径**。现已双向断言（`task-191`）。
2. **事件名**：`events.rs` 的常量 ↔ `ipc.ts::EVENTS` 的值原先**没有任何测试比对**
   ⇒ 名字漂移只会表现为「**静静地收不到事件**」（正是 `task-192` 修的那个病）。现为**严格字面相等**
   （9 = 9，**不引入登记表/白名单** —— 差集已不存在，加那种机制只会将来腐烂）。
3. **字段按子系统拆分**：`check_error` / `checked_at` 是客户端 / 核心 / geo **三路共用**，
   而 `check_error_app` / `checked_at_app` 只关于客户端 ⇒ 「核心/geo 失败」不再被当成「客户端不知道」。

### 11.4 门禁证据（Lead 在隔离 worktree，`BUILD_LOCK_STRICT=1 ./scripts/check.sh --no-release-build`）

| 时点 | 结果 | 覆盖 |
|---|---|---|
| `674f25d` | `GATE_EXIT=0` | `task-188` 本体 |
| `e981d7d` | `GATE_EXIT=0` | 并行线 rebase 后的远端 tip |
| `e1de2ec` | `GATE_EXIT=0` | `task-192`（分支头） |
| `f3cbd48` | `GATE_EXIT=0` | `task-191`（分支头） |
| **`d12bee9`** | **`GATE_EXIT=0`** | **`task-191` 落 main 的那个 sha** |

**后续几张卡的落法（共享工作树被并行线的 rebase 占着期间）**：在**隔离 worktree** 里跑完整 `check.sh`
→ 分支头全绿 → cherry-pick/快进落 `main` → 再按**路径**核对「已验分支头 == 已推提交」
（整树哈希必然不同：同一时刻并行线又推了 `site/**`）。`task-195` 是 Lead 自己在 `leadgate182` 上做的，
所以直接对**提交** `2f7d484`（后端三处口径）与 `03bd1ff`（banner 标注）各跑了一次完整门禁，**均 `GATE_EXIT=0`**。

`task-188` 自报：`cargo test --workspace` = 17 个二进制 **826 passed / 0 failed / 9 ignored**、`clippy` 0/0，
**5 条反向敏感性全部红**（去掉周期守卫、失败清 `latest_app`、只允许走节点、失败说「已是最新」、每次失败都刷屏）。
`task-191`：desktop lib **271/0/5**、`type_contract` **8 passed**（两条新契约）、**5 条突变全红**。

### 11.5 同步协作事故：共享工作树里的 `rebase --autostash`

并行工作流在**同一工作树**里跑了 1.5 小时以上的 `rebase --autostash`（reflog 里还从一次 `reset` 里找回被丢掉的提交），
期间仍在改 `site/**`。实况与影响：

* 我们一位队友**已经在途的 36 行**被 autostash 收走（`87047d7`），主树里的文件「突然不见」；
  他因此拿到一次 **`VITEST_EXIT=1` / `TSC_EXIT=2` 的假红**（那是半截树，不是他的代码）；
* 应对：**全部改成「隔离 worktree + 独立分支 + 我在分支头上跑门禁 → 快进落 main」**，
  不再让我们的提交去等那条 rebase（`task-192`、`task-191` 都是这么落的）；
* 期间 `origin/main` 被并行线推进得非常快，落 main 需要「取最新 main → cherry-pick → 立即推」的重试
  （实测 1 分钟内被拒过一次）。

**教训（写进提交纪律）**：**rebase 要在独立 worktree 里做**（或 rebase 前让写入者停手）。
一次 `rebase --autostash` 就能把别人的在途改动**静默移出视野**，产生「谁改的文件不见了」这类假红。

### 11.6 诚实清单（本节）

* **真机未验证**：「启动 20 s 后自动联网查一次」「每 6 h 再来一轮」都是**读码 + 纯函数单测 + 常量**层面的结论，
  本环境跑不了真实 App 启动流程（需要 GUI 会话 + Tauri 运行时）；真机 WKWebView 同样未验。
* **GitHub 匿名配额**：未认证 **60 次/小时、按 IP**；经节点出去时等于和**整台节点的用户共用**那份配额
  （这正是 20 s 延迟 + 6 h 周期 + `should_check` 第二道守卫的理由）。403 由既有的 `gh_status_error` 翻成「限流」人话。
* 「单轮 > 6 h 导致 `interval` Burst 补发」是**按 tokio 语义推断、未实测**。
* **core/geo 的失败现在只出现在「合并 banner」里**（`03bd1ff` 起会标注是哪一类），
  **还没有**各自独立的展示位置；`checked_at_app` 也仍与「上次成功那一次」的时刻分不开
  （失败也会更新它）⇒ 这两条留给后续 UI 卡，别把它当成「已经呈现完整」。
* 「三子系统**并发**写入时的中间态可见性」未覆盖：两个自动检查可能并发，快照读到的可能是
  「一个已写、另一个未写」的中间态（本波未加锁收口，只在报告里点名）。
* 本节的门禁都是**时点凭据**（并行线提交极密），不是「此刻 HEAD 一定绿」。
