...
///
/// ⚠️ **回环不在这个列表里**（`127.0.0.0/8`、`::1/128`）—— 原因见数组里的说明。
pub fn default_bypass_networks() -> Vec<Cidr> {
    [
        ...
        // 这里**没有** 127.0.0.0/8。
        //
        // 回环从来不需要「绕过隧道」：内核本来就把 `127.0.0.0/8` 指向 `lo0`，
        // 而它比捕获用的 `0.0.0.0/1` **更具体**（`/8` > `/1`）⇒ TUN 卷不走回环。
        // 一旦把它列进来，它会被当成普通 IPv4 旁路网段处理、**指向物理网关**
        // （见 `xt-tun/src/plan.rs` 的 bypass 循环）—— 后果是 `127.0.0.2` 及以上
        // 被送去局域网路由器，换网后还指向旧网关。
        // **实测**：用户机器上真的出现过 `127 → 192.168.0.1 UGSc en0`
        // （而 `127.0.0.1/32 → lo0` 一直在，所以 `.1` 侥幸没事）。
        //
        // `plan.rs` 另有一层**防御**：请求里就算带了回环目的地（网段或主机），
        // 也不会被装上经由网关的路由 —— 见 `build_plan` 里的 `is_loopback` 跳过。
    ]
    .iter()
...
mod tests {
    ...
    /// 回归：**回环不在 bypass 列表里**（task-83）。
    ///
    /// 它在列表里时会被当成普通旁路网段、**指向物理网关** ——
    /// 实测用户机器上出现过 `127 → 192.168.0.1 UGSc en0`：
    /// `127.0.0.2` 及以上会被送去局域网路由器（`.1` 靠自带的 `/32 → lo0` 才没事）。
    /// 而回环本来就不需要旁路：内核已把它指向 `lo0`，且比捕获用的 `0.0.0.0/1` 更具体。
    #[test]
    fn default_bypass_networks_excludes_loopback() {
        for c in default_bypass_networks() {
            assert!(
                !c.addr.is_loopback(),
                "回环不该在 bypass 列表里（列进来只会被装上网关路由）：{c}",
            );
        }
    }
