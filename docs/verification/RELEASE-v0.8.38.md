...
| `task-193` | *进行中* | chip 改用客户端专属字段（现在核心/geo 的失败会把「客户端已是最新」说成「不知道有没有新版本」——under-claim） |
...
* **`task-193` 尚未落地**：在那之前，仪表盘 chip 仍用合并字段 `check_error` 下客户端结论 ⇒
  「核心/geo 检查失败 + 客户端已最新」会显示成「不知道有没有新版本」（under-claim）。
