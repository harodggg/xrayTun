...
**这一段的口径头（按项目约定，数字必须连它一起引）**：

| 口径项 | 值 |
|---|---|
| 身份 | `scripts/net-metrics.py`，口径头 v2（2026-09-22） |
| 切（窗口） | 全部记录 → **2026-09-22 14:21:32 +0800** |
| 移动标记 | N（命中） = **305,742** 条；T = **2026-09-22 14:21:32**（**都随日志增长移动**） |
| 解（解析） | `raw_decode` 循环（一行可含多个对象）；**全文件扫描** 409,067 行 → 409,068 个对象；多对象行 4 / 截断 0 / 非 JSON 0 / 空行 3 |
| 匹配（去重） | 键 = **`(ts_unix, message)`**，先见者胜；顺序 `app.1.jsonl` → `app.jsonl`；**丢弃重复 39,936 条（全文件扫描口径）**，其中落在本窗口内 **39,936** 条 |
| 单位 | 时间 = `ts_unix`(秒)；耗时 = 消息内微秒时间戳；探针轮 = 相邻间隔 >5s；探针超时 = 6s |
| 行内上报 | `message 含 LF 的记录数` = **0** |
| **选择内容指纹** | `sha256:3e7c0c61ef53e6d4fbb1a2f7a89c6e6fa8e43bc188fcfd53a7df14795ec5b3cb`（**选择集**指纹，不是文件指纹） |

> 指纹可复算（不需要本脚本）：`--export` 导出的就是被指纹的字节，`shasum -a 256` 的结果与上面一致（见 `NET-METRICS.md` §3.3）。
