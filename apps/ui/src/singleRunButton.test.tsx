/**
 * task-72：同一屏上只能有**一个**「连接/断开」控件；「本应用不设系统代理」徽章只在
 * 「核心运行中 + 系统代理模式」出现。
 *
 * # 防的两个故障
 *
 ...
 *    点那一颗必须仍然调用 `stop`。
 * 2. **「本应用不设系统代理」徽章常驻**：`SystemProxy` 是 `#[default]`，新用户一打开就是
...
describe("task-72 ①：「本应用不设系统代理」徽章三态", () => {
  const badge = () => screen.queryByText(/本应用不设系统代理/);
