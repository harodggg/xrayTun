...
| 3 | task-61 192 行重复 | 删掉重复块 → 真浏览器**逐元素比较计算样式**（`.chain*` 8 行 × 1280/900 × 20 个属性） | **未能攻破**（差异 0）；但**该重复在 HEAD 仍存在**，且我量到的是 **88 行**不是 192 行 | 元素数 `.chain`=1、`.chain__row`=8…；**diff=0**；`styles.css` 2577→2489 行 |
...
**先说两个与卡里描述不符的事实**（都是实测）：
1. **该重复块在 HEAD `bdeee46` 仍然存在**（`git log -- styles.css` 里没有去重提交）；
   我能找到的是**逐行相同 88 行**，两份分别在 `styles.css:2114` 与 `:2344`
   （内容：`.chain*` / `.verdict*` 共 11 个选择器）。卡里的「192 行」我没能复现出这个数字。
2. 我做的不是「验证作者删得对」，而是**替这项改动做证伪实验**（在 /tmp 里删掉第二份）：

| 检查 | 结果 |
|---|---|
| 两份的级联上下文 | 都在**花括号深度 1**（同一层，不在 `@media` 内） |
| 两份之间是否有规则提到这些 class | **没有**（142 行间隔里 `.chain*`/`.verdict*` 出现 0 次） |
| 真浏览器计算样式 diff（`?preview=1&state=connected&view=topology`；`.chain`/`.chain__row`/`__idx`/`__tag`/`__conds`/`__arrow`/`__out`；1280 与 900 两个视口；20 个属性） | **差异 0 条**；元素数完全一致（`.chain`=1、其余各 8） |

**没能覆盖的**：`.verdict*`（目的判定结果卡片）在预览里没有渲染（元素数 0），
那部分只有静态结论；也没有测其它页面/更多视口。

---

## 4. 我发现但没在本卡修的

1. **`styles.css` 的 88 行重复仍在 HEAD**（`2114` 与 `2344`）—— 若 task-61 声称已删，
   那是**没落地**（我的证伪实验同时给出了「删掉是安全的」的证据：真浏览器 diff 0）。
